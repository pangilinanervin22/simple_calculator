...
public class StringManipulation {
    ...
    public String updateCurrentString(String text, String input) {
        ...
        boolean inputModuloWrong = input.equals("%") && (!lastCharIsNumber(text) || text.endsWith("%"));
        ...
        boolean inputNegativeTwice = input.equals(negativeSymbol) &&
                (text.endsWith(negativeSymbol) || text.endsWith(minusSign) || lastCharIsNumber(text));
        boolean inputDecimalWithSpecial = text.endsWith(".") && (input.equals("(") || isCharOperator(input) || input.equals(negativeSymbol));

        if (input.equals("") || inputOperationTwice || inputNegativeTwice || inputMinusWithNegative || inputModuloWrong ||
    ...
    public String addPositiveNegative(String text) {
        char[] chars = text.toCharArray();
        String temp = "";

        int index = text.length() - 1;
        while (index > 0 && lastCharIsNumber(String.valueOf(chars[index])) )
            temp = temp + chars[index++];

        return text.substring(0, index) + "-" + temp;
    }
